...
import { 
  RotateCcw, AlertCircle, TrendingUp, ArrowRight, Check, Zap, Radio, Users, 
  ShieldAlert, Activity, Trash2, Sparkles, Target, Mic, MicOff, Volume2, 
  Briefcase, Scale, ListTodo, FileText, ChevronRight, Eye, 
  Download, ExternalLink, FileEdit, Pause, PlayCircle, CornerUpLeft, Globe, 
  CloudUpload, CheckCircle2, Loader2, Link2, Image as ImageIcon, Video as VideoIcon,
  ChevronLeft, Layout, Languages, Presentation as PresentationIcon
} from 'lucide-react';
import { GoogleGenAI } from "@google/genai";
import { AppState, AnalysisResponse, UserProfile, HistoryItem, LiveTranscription, LiveActions, ExportRequest, SupportedLanguage, LANGUAGE_NAMES, PresentationResponse } from './types';
import { startAnalysis, decodeAudioData, connectLive, createBlob, decode, generateStrategicPresentation } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  useEffect(() => { localStorage.setItem('hiru_history_v4', JSON.stringify(history)); }, [history]);
  useEffect(() => { localStorage.setItem('hiru_profile_v4', JSON.stringify(profile)); }, [profile]);
  ...
  const handleGeneratePresentation = async () => {
    setAppState(AppState.LOADING);
    try {
      const p = await generateStrategicPresentation(
        inputText || result?.execution.content || "",
        profile,
        presentationLang,
        targetCulture
      );
      setPresentationResult(p);
      setAppState(AppState.PRESENTATION); // We reuse the state to show the result
    } catch (e) {
      setAppState(AppState.RESULT);
    }
  };
      ...
      const sessionPromise = connectLive({
        ...
        onmessage: async (message) => {
          ...
          if (message.serverContent?.inputTranscription) {
            setLiveTranscriptions(prev => [...prev, { text: message.serverContent!.inputTranscription!.text, type: 'user', timestamp: Date.now() }]);
          }
  ...
  const handleExport = async (request: ExportRequest) => {
    ...
    try {
      // Create new GoogleGenAI right before call to ensure up-to-date API key
      ...
      if (request.payload.format === 'IMAGE') {
        ...
        // Appending key to download link is required for direct fetches/renders
        if (op.response?.generatedVideos?.[0]?.video?.uri) {
          resultUrl = `${op.response.generatedVideos[0].video.uri}&key=${process.env.API_KEY}`;
        }
      } else {
      ...
      // Re-trigger key selection if "Requested entity was not found" error occurs (Veo/Gemini 3 Pro)
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-all duration-700 ${isLiveActive ? 'bg-zinc-950 text-white' : 'bg-brand-cream/20'}`}>
      <nav className={`h-24 px-12 flex items-center justify-between sticky top-0 z-50 backdrop-blur-xl border-b transition-all ${isLiveActive ? 'bg-zinc-950/90 border-white/5' : 'bg-white/95 border-brand-soft shadow-sm'}`}>
        <div className="flex items-center gap-6 cursor-pointer" onClick={() => setAppState(AppState.IDLE)}>
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl shadow-lg ${isLiveActive ? 'bg-brand-soft text-brand-green' : 'bg-brand-green text-white'}`}>H</div>
        ...
        <div className="flex gap-4 items-center">
          <button onClick={isLiveActive ? stopLiveSession : startLiveSession} className={`px-8 py-3.5 rounded-2xl font-black text-[10px] uppercase flex items-center gap-3 transition-all ${isLiveActive ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'bg-brand-green text-white shadow-2xl hover:scale-105 active:scale-95'}`}>
                   ...
                   <div className="bg-zinc-900/50 rounded-[40px] border border-white/5 p-8 flex flex-col items-center gap-6 shadow-2xl backdrop-blur-md">
                     ...
                     <div className="grid grid-cols-2 gap-2 w-full">
                       {['PAUSA', 'CONTINUAR', 'CORREGIR', 'REPETIR', 'DESHACER', 'EXPORTAR'].map(cmd => (
                         <button key={cmd} onClick={() => sendCommand(cmd)} className="py-3 bg-zinc-800/60 border border-white/5 rounded-xl text-[8px] font-black uppercase hover:bg-brand-soft hover:text-brand-green transition-all shadow-sm">
                           {cmd}
                         </button>
                       ))}
                     </div>
                   ...
                   <div className="flex-grow bg-zinc-900/20 rounded-[40px] border border-white/5 p-8 overflow-y-auto flex flex-col gap-4 custom-scrollbar">
                      ...
                      {liveTranscriptions.slice(-10).map((t, i) => (
                        <div key={i} className={`flex ${t.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[90%] p-4 rounded-[24px] text-xs font-bold shadow-lg ${t.type === 'user' ? 'bg-brand-soft text-brand-green rounded-tr-none' : 'bg-zinc-900/60 text-zinc-400 rounded-tl-none border border-white/5'}`}>
                            {t.text}
                          </div>
                        </div>
                      ))}
                ...
                <div className="lg:col-span-9 flex flex-col gap-6 h-full overflow-hidden">
                     ...
                     <div className="flex items-center justify-between border-b border-white/5 pb-6">
                        ...
                        {liveActions?.preview_paging && (
                          <div className="flex items-center gap-3">
                             <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">PARTE {liveActions.preview_paging.current_part} / {liveActions.preview_paging.total_parts}</span>
                          </div>
                        )}
                     </div>
                     ...
                     {!previewContent ? (
                       ...
                       <div className="space-y-10 animate-in fade-in slide-in-from-right-10 duration-700">
                         ...
                         <div className="flex justify-center gap-6">
                            {liveActions?.preview_paging?.actions.map(act => (
                              <button key={act.id} onClick={() => sendCommand(act.voice_hint)} className="px-10 py-5 bg-zinc-800/80 hover:bg-brand-soft hover:text-brand-green rounded-[30px] text-[10px] font-black uppercase border border-white/5 transition-all flex items-center gap-4 group">
                           ...
                           {exportState.status === 'processing' ? (
                             ...
                             <>
                               ...
                               {exportState.type === 'IMAGE' && exportState.url ? (
                                  ...
                                  <a href={exportState.url} target="_blank" rel="noopener noreferrer" className="px-14 py-6 bg-brand-soft text-brand-green rounded-full font-black uppercase text-xs tracking-widest shadow-2xl flex items-center gap-4 hover:scale-105 transition-all">
                             ...
                             <div className="text-red-500 flex flex-col items-center gap-4">
                               ...
                               <button onClick={() => setExportState({ status: 'idle' })} className="text-xs font-black uppercase underline">Reintentar</button>
                             </div>
                           )}
                        </div>
                     )}
                   ...
                   <div className="bg-zinc-900/40 p-10 rounded-[45px] border border-white/5 flex flex-col gap-8 backdrop-blur-md">
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        {(liveActions?.next_actions || []).map((action, i) => (
                          <button key={i} onClick={() => sendCommand(action.voice_hint)} className="px-6 py-6 bg-zinc-800/40 border border-white/5 hover:border-brand-soft rounded-[28px] text-[9px] font-black uppercase text-zinc-500 hover:text-white transition-all text-center flex flex-col items-center gap-3">
          ...
          <div className="max-w-2xl mx-auto bg-white p-16 rounded-[70px] shadow-2xl space-y-12 animate-in zoom-in-95 mt-10 border border-slate-100">
            ...
            <div className="space-y-10">
              <div className="space-y-4">
                <label className="block text-[11px] font-black uppercase tracking-widest text-slate-400 ml-6 flex items-center gap-3"><Briefcase className="w-4 h-4" /> Perfil Profesional</label>
                <textarea className="w-full p-10 bg-brand-cream/40 rounded-[50px] outline-none font-bold text-slate-700 resize-none h-48 border-2 border-transparent focus:border-brand-green/10 transition-all shadow-inner text-lg" placeholder="Ej: Consultora Senior de Estrategia Digital..." value={profile.situation} onChange={e => setProfile({...profile, situation: e.target.value})} />
              </div>
            </div>
            <button onClick={() => setAppState(AppState.IDLE)} disabled={!profile.situation} className="w-full py-8 bg-brand-green text-white rounded-[50px] font-black uppercase tracking-[0.3em] shadow-2xl hover:scale-105 transition-all disabled:opacity-50">Activar Hiru Vocal Pro</button>
          </div>
        )}
            ...
            <div className="bg-white p-20 rounded-[100px] shadow-2xl border-t border-white space-y-16">
              <div className="grid md:grid-cols-3 gap-10">
                {['Cliente', 'Proveedor', 'Libre'].map(type => (
                   <button key={type} className="flex flex-col items-center gap-6 p-12 bg-brand-cream/30 rounded-[50px] border-2 border-transparent hover:border-brand-green/20 hover:bg-white transition-all group shadow-sm">
              ...
              <div className="flex flex-col items-center gap-10 border-t pt-16">
                <button onClick={startLiveSession} className="px-40 py-9 bg-brand-green text-white rounded-[60px] font-black uppercase tracking-[0.4em] shadow-2xl hover:scale-105 transition-all flex items-center gap-8 text-2xl group">
          ...
          <div className="max-w-7xl mx-auto space-y-20 pb-32 animate-in slide-in-from-bottom-12">
            <div className="flex justify-between items-center border-b pb-10">
               ...
               <div className="flex gap-4">
                  <button onClick={() => setAppState(AppState.PRESENTATION)} className="flex items-center gap-6 px-12 py-6 bg-brand-soft text-brand-green rounded-[40px] font-black text-sm uppercase hover:bg-brand-green hover:text-white transition-all shadow-2xl group">
                    <PresentationIcon className="w-6 h-6" /> Teatro Visual
                  </button>
                  <button onClick={startLiveSession} className="flex items-center gap-6 px-12 py-6 bg-brand-green text-white rounded-[40px] font-black text-sm uppercase hover:bg-brand-soft hover:text-brand-green transition-all shadow-2xl group">
            ...
            <div className="p-20 bg-white rounded-[100px] shadow-3xl border border-slate-100 space-y-12">
               <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-4">
                     <button className="p-4 bg-brand-cream rounded-2xl hover:bg-brand-soft transition-all"><FileText className="w-5 h-5 text-brand-green" /></button>
                     <button className="p-4 bg-brand-cream rounded-2xl hover:bg-brand-soft transition-all"><ImageIcon className="w-5 h-5 text-brand-green" /></button>
                     <button className="p-4 bg-brand-cream rounded-2xl hover:bg-brand-soft transition-all"><VideoIcon className="w-5 h-5 text-brand-green" /></button>
                  </div>
               </div>
               <div className="bg-brand-cream/40 p-16 rounded-[60px] font-medium text-2xl whitespace-pre-wrap leading-relaxed shadow-inner">
            ...
            {!presentationResult ? (
              <div className="bg-white p-20 rounded-[80px] shadow-2xl border border-slate-100 space-y-12 text-center">
                <div className="flex flex-col items-center gap-6">
                  ...
                  <p className="text-slate-500 font-bold max-w-xl mx-auto">Adapta tu presentation estratégica al idioma y contexto cultural de tu audiencia.</p>
                </div>
                ...
                <div className="flex flex-col items-center gap-6 pt-10">
                  ...
                  <button onClick={() => setAppState(AppState.RESULT)} className="text-xs font-black uppercase text-slate-400 hover:text-brand-green underline decoration-brand-green/20 underline-offset-8 transition-all">Volver al Análisis</button>
                </div>
              </div>
            ) : (
              <div className="space-y-12">
                ...
                <div className="bg-white p-20 rounded-[80px] shadow-3xl border border-slate-100 min-h-[600px] flex flex-col justify-between relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-12 opacity-5"><PresentationIcon className="w-48 h-48" /></div>
                ...
                <div className="flex justify-between items-center px-10">
                  ...
                  <div className="text-xl font-black brand-font text-brand-green">{currentSlide + 1} / {presentationResult.slides.length}</div>
                  <button 
      ...
      <footer className={`h-28 border-t flex items-center justify-center transition-all ${isLiveActive ? 'bg-zinc-950 border-white/5' : 'bg-white'}`}>
        <p className={`text-[10px] font-black uppercase tracking-[0.8em] ${isLiveActive ? 'text-zinc-800' : 'text-brand-green'}`}>HIRU PRESTAKUNTZA &copy; 2025 | CLOUD MULTIMODAL ENGINE</p>
      </footer>
    </div>
  );
};
